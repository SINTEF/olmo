...
def query_influxdb(client, measurement, variable, timeslice, downsample, approved='yes'):
    ...
    if downsample:
        q = f'''SELECT mean("{variable}") AS "{variable}" FROM "{measurement}" WHERE {timeslice} "approved" = '{approved}' GROUP BY {downsample}'''
    else:
        q = f'''SELECT "{variable}" FROM "{measurement}" "approved" = '{approved}'  WHERE {timeslice}'''
